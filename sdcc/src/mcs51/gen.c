  ...
  if (aop->aopu.aop_immd.from_cast_remat)
    {
      ...
      if (ptr_type == GPTYPE_CODE)
        dbuf_printf (&dbuf, "#((%s + 0x800000) >> 16)", aop->aopu.aop_immd.aop_immd1);
      else
...
/*-------------------------------------------------------------------*/
/* emit_dec_dptr - emit code to add/subtract a constant to/from dptr */
/*-------------------------------------------------------------------*/
static void
          ...
          if (aop->aopu.aop_immd.from_cast_remat && aop->aop_litimmd_is_gptr && offset == GPTRSIZE - 1)
          ...
          else if (offset)
            {
              ...
              if (offset == GPTRSIZE - 1)
                dbuf_printf (&dbuf, "#((%s + 0x800000) >> %d)", aop->aopu.aop_immd.aop_immd1, offset * 8);
              else
...
genCast (iCode * ic)
{
  ...
  /* if the either is of type pointer.
     note that a function can be treated as a function-pointer here, too.  */
  if ((IS_PTR (ctype) || IS_PTR (rtype) || IS_FUNC (rtype)) && !IS_INTEGRAL (rtype))
    {
      int p_type;
          ...
          /* the third byte depending on type */
          {
            ...
            if (gpVal == -1)
            ...
            else
              {
                ...
                // let opGet handle it.
                // this works only if the source can actually supply a third byte:
                // an immediate symbol (aop_litimmd_is_gptr) or an operand that
                // really has one (e.g. banked function pointer).  a plain 2-byte
                // code pointer in registers/stack has lost the bank byte, so
                // fall back to the constant gp byte for it.

                if (gpVal == GPTYPE_CODE
                    && (AOP_SIZE (right) > offset || AOP (right)->aop_litimmd_is_gptr))
                  opPut (result, opGet (right, offset, FALSE, FALSE), offset);
                else
                  {
                    struct dbuf_s dbuf;
